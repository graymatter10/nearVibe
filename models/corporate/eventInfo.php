...
function countTicketsSoldByOrganizer($uid)
{
    $conn=dbConnection();

    if($conn)
    {
        $sql="SELECT COUNT(*) AS total FROM ticket t JOIN events e ON t.eid=e.eid WHERE e.uid=?";
        $stmt=mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, 'i', $uid);
        mysqli_stmt_execute($stmt);
        $result=mysqli_stmt_get_result($stmt);
...
function sumRevenueByOrganizer($uid)
{
    $conn=dbConnection();

    if($conn)
    {
        $sql="SELECT SUM(e.tprice) AS total FROM ticket t JOIN events e ON t.eid=e.eid WHERE e.uid=?";
        $stmt=mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, 'i', $uid);
        mysqli_stmt_execute($stmt);
        $result=mysqli_stmt_get_result($stmt);
